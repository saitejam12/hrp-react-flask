...
const LEAVE_ITEMS: MenuItem[] = [
  { label: 'Apply Leave',   path: '/leave/apply',      icon: <LeaveIcon /> },
  { label: 'Leave Balance', path: '/leave/balance',    icon: <LeaveBalanceIcon /> },
  { label: 'Approvals',     path: '/leave/approvals',  icon: <LeaveApprovalIcon />, roles: ['admin', 'hr', 'owner'] },
  { label: 'Calendar',      path: '/leave/calendar',   icon: <LeaveCalendarIcon /> },
  { label: 'Encashment',    path: '/leave/encashment', icon: <EncashmentIcon /> },
];
  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav">
        ...
        <div className="nav-section-title">Leave</div>
        <ul className="nav-list">
          {LEAVE_ITEMS.filter((item) => !item.roles || item.roles.includes(user?.role || '')).map((item) => (
            <li key={item.path}>
              <a
                href={item.path}
                className={`nav-link ${currentPath === item.path ? 'active' : ''}`}
